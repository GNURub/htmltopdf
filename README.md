...
- Tag, `.class`, `#id`, compound selectors (`section.card.primary`), descendant selectors (`.shell .card h1`), attribute selectors (`[data-state]`, `[data-state="active"]`, `~=`, `|=`, `^=`, `$=`, `*=`, explicit ASCII case flags `i`/`s`), structural pseudo-classes, `:not(...)`, descendant `:has(...)`, first-pass CSS nesting (`& .child`), and `:where(...)`/`:is(...)` selector-list functions expanded across all arguments with browser-like cascade specificity for modern framework CSS
- Inline `style` attributes and local `<link rel="stylesheet" href="...">` stylesheets resolved from the HTML file directory in the CLI
- CSS parser compatibility for build-tool comments (`/* ... */`), case-insensitive CSS function/at-rule names where supported, and semantic, case-insensitive `!important` author-cascade priority, without letting comments poison the following selector
- `display: none|block|inline|inline-block|contents|flow-root|table`; first-pass `flex`/`inline-flex`/`grid` layout with `gap`, `order`, `align-self`, explicit/intrinsic flex item widths, shrink-to-content inline-flex boxes, `justify-content`, `align-items`, `grid-template-columns` fractional `fr` tracks, fixed tracks, `repeat(n, ...)`, and first-pass `place-items`
- First-pass `overflow: hidden|clip` clipping for rectangular and rounded containers
- Browser-like non-painted content handling for `hidden`, `visibility: hidden`, `template`, `content-visibility: hidden`, and unsupported embedded media containers such as `iframe`; `aria-hidden="true"` is intentionally accessibility-only and remains visually rendered, matching browsers
- Box model: margins, padding, `width`, `min-width`, `max-width`, `height`, `min-height`, `max-height`, borders, and non-layout `outline`/`outline-offset`, plus first-pass logical aliases such as `inline-size`, `min-block-size`, `max-block-size`, `padding-inline`, `margin-block`, and `border-inline-start`
...
- First-pass positioned layout for `position: relative|absolute|fixed|sticky` with `top`, `right`, `bottom`, `left`, logical `inset-*` aliases, and same-page positioned-sibling `z-index` paint ordering; absolute/fixed boxes are painted out of normal flow
- Modern length functions for layout-critical properties: `calc(...)`, `min(...)`, `max(...)`, `clamp(...)`, percentages, `px`, `pt`, `mm`, `cm`, `in`, `rem`, `em`, `ch`
- Inherited text styling: `font-size`, `line-height`, `font-family` mapped to built-in sans/serif/monospace PDF fonts, `font-weight` including heavy numeric buckets, `font-style: normal|italic|oblique` with lightweight PDF slanting, first-pass `font-variant-numeric: tabular-nums`/`font-feature-settings: "tnum"`, `text-decoration`/`text-decoration-line` for underline and line-through, `letter-spacing`, `text-transform`, `<br>` hard line breaks, `white-space`, `text-overflow: ellipsis` for hidden nowrap labels, `overflow-wrap`, `color`, `text-align`
- Automatic embedding of local TrueType fonts as CID/Identity-H fonts with `ToUnicode` maps whenever text leaves WinAnsi, preserving Unicode extraction without inflating ordinary ASCII/Latin-1 PDFs; `HTMLPDF_EMBED_FONTS=1` remains available to force embedding for every font
- Generated content: textual `::before`/`::after` plus empty `content: ""` pseudo-element boxes for decorative rules, pills, dots, overlays, gradients, borders, rounded corners and shadows
- Media/object sizing: local PNG/JPEG `<img>` and background images with first-pass `object-fit`, `object-position`, `aspect-ratio`, and `background-size`/position handling
- Backgrounds and effects: solid/alpha colors, first-pass angle-aware multi-stop `linear-gradient(...)` including positioned stops and linear layers inside compound `background`, document-level body/html backgrounds bounded to the print content box with white print-margin masks, first-pass text `transform: rotate(...)`, layered first-pass blur approximation for comma-separated `box-shadow` stacks, and `filter: drop-shadow(...)` mapped to the lightweight shadow model
- First-pass inherited subtree opacity approximation via `opacity`
- Print-safe compatibility acceptance for common screen/framework properties that do not affect static PDF output, including `cursor`, `touch-action`, `user-select`, `pointer-events`, `transition`, `animation`, `will-change`, `appearance`, `backdrop-filter`, font smoothing/rendering hints, `font-kerning`, `font-optical-sizing`, `font-synthesis`, `color-scheme`, `isolation`, `container-type`, scroll/overscroll hints, `accent-color`, `caret-color`, `resize`, and `print-color-adjust`
- Borders/tables: `border`, side borders (`border-left`, `border-bottom`, etc.), `border-width`, `border-color`, `border-radius`, style-aware table cell borders, `vertical-align: top|middle|bottom|baseline` for table cells, plus first-pass `border-collapse: collapse` table row/border compaction
- At-rules and print CSS: first-pass `@media print`, page-width `@media (max-width: ...)`/`(min-width: ...)` and modern range syntax such as `(width >= 40rem)`, positive `@supports (...)`, first-pass cascade-layer priority for `@layer ... { ... }` / `@layer reset, components;` where declared named-layer order is honored, unlayered normal rules outrank layered normal rules, and important layered declarations reverse layer order, positive `@container ... { ... }`, scoped `@scope (.root) { ... }`, `@page size`, `@page margin`, mixed page sizes in combined PDFs, page-break hints, and repeated table headers across pages
