...
- Optional `HTMLPDF_EMBED_FONTS=1` path for embedding local TrueType fonts as CID/Identity-H fonts with `ToUnicode` maps, preserving Unicode extraction and avoiding fake Type1 font names when portability matters; it is still opt-in while browser-parity metrics are tuned
